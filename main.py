...
async def main():
    ...
    while True:
        ...
        if scene == SCENE_TITLE:
            ...
            draw_villager_poll_scene(villager_poll_index=
                villager_poll_index)
            ...
            scene = draw_discuss_scene(discuss_start_time,
                most_suspicious_player)
        elif scene == SCENE_PLAYER_FOR_POLL:
            if player_index_for_poll < len(players) - 2:
                draw_player_for_poll_scene(player=players[
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if scene == SCENE_TITLE:
                    ...
                    scene = update_role_scene(player=players[
                        player_index])
                elif scene == SCENE_VILLAGER_POLL:
                    ...
                    scene, is_grave = update_fortune_teller_select_scene(
                        is_grave)
                elif scene == SCENE_FORTUNE_TELLER:
                    ...
                    scene, player_index = update_werewolf_scene(
                        player_index)
                elif scene == SCENE_DISCUSS:
